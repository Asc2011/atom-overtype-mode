...
cmd.pasteLikeLawrence = function() {
  var clipboardText, column, editor, end, error, i, idx, lastLine, lastLineIdx, leftPrefix, len1, lineEnding, lines, newLine, newLines, oldLine, pristineBuffer, range, ref, row, spc;
  ...
  if (column === 0) {
    ...
    newLines = [];
    for (idx = i = 0, len1 = lines.length; i < len1; idx = ++i) {
      newLine = lines[idx];
  ...
  try {
    ...
    range = [[row, 0], [lastLineIdx, lastLine.length]];
    end = editor.setTextInBufferRange(range, newLines.join(lineEnding)).end;
