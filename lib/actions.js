var cmd, log, myClipboard, overwriteSelection;
...
cmd.duplicateLines = function() {
  ...
  if (!(editor = this.active())) {
    this.log("cmd.duplicateLines -> .insertNewlineBelow");
    return this.activeEditor().insertNewlineBelow();
  }
  this.log("cmd.duplicateLines editor='" + editor.id + "'");
...
cmd.pasteLikeLawrence = function() {
  ...
  this.log("cmd.pasteLikeLawrence starts");
  ...
  if (lines.length === 0) {
    this.log("cmd.pasteLikeLawrence no-content", 1);
    return;
  }
  ...
  try {
    ...
    return this.log('pasteLikeLawrence:: finished without error.', 1);
  } catch (error1) {
    error = error1;
    this.log("cmd.pasteLikeLawrence:: error was " + error, 3);
  ...
  return editor.mutateSelectedText((function(_this) {
    return function(sel, idx) {
      ...
      if (sel.isEmpty()) {
        ...
        _this.log("cmd.backspace char='" + char + "'");
  ...
  return editor.mutateSelectedText((function(_this) {
    return function(sel, idx) {
      ...
      if (!editor.hasMultipleCursors()) {
        _this.log("cmd.delete mode=single char='" + char + "'");
      ...
      range = sel.getBufferRange();
      _this.log("cmd.delete inserts ' ' range=" + range);
...
myClipboard = function(lineEnding) {
  var cb;
  cb = {
...
cmd.peekForVerticalPaste = function(editor) {
...
cmd.peekBeforePaste = function() {
  ...
  return editor.mutateSelectedText((function(_this) {
    return function(sel, idx) {
      var selLen, start;
      _this.log("cmd.peekForPaste:: selection-" + idx);
      if (!sel.isEmpty()) {
        sel.clear();
...
cmd.smartInsert = function() {
  ...
  hasSpacedArea = function(line, start) {
    ...
    while (start !== end) {
      char = line[start];
      this.log("cmd.smartInsert:: start=" + start + " char='" + char + "' c=" + c);
      if (char == null) {
        break;
      }
      if (char === ' ') {
